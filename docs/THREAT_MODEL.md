...
KANDOR manages diagnostic agents only on systems the operator is authorized to inspect. The server host and deployment administrators are trusted but still auditable; browsers, networks, request data, enrollment tokens in transit, and enrolled endpoints may be compromised.
...
- Host metadata, diagnostic results, task history, settings, and audit events.
...
KANDOR does not claim to protect a fully compromised server/database administrator, provide hardware-backed endpoint attestation, detect all hostile endpoint behavior, replace EDR/SIEM, or deliver forensic chain-of-custody guarantees. It does not execute arbitrary code and must not be extended into offensive or covert tooling.

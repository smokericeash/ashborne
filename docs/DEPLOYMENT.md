...
The frontend and backend share a dedicated host-facing proxy network. That network must **not** set `internal: true`; Docker otherwise blocks host access even when ports are published. The separate `ashborne-data` and `ashborne-agent` networks remain internal. Compose gives the frontend the address declared by `ASHBORNE_FRONTEND_PROXY_IP`, and Uvicorn trusts forwarded headers only from the matching `ASHBORNE_FORWARDED_ALLOW_IPS`. If `ASHBORNE_PROXY_SUBNET` must change to avoid a local route conflict, update all three values together. Production ingress addresses must be configured explicitly; never use a wildcard while the API is directly reachable.

On the first start of a fresh PostgreSQL volume, `docker/postgres/init-app-role.sh` creates the `ASHBORNE_DB_USER` login with `NOSUPERUSER`, `NOCREATEDB`, and `NOCREATEROLE`. The backend and its Alembic migration use that account; `POSTGRES_USER` remains a separate database-owner account and is not injected into the backend container. Initialization scripts do not run again for an existing volume, so later credential rotation requires a controlled database role update (or an intentional lab-volume reset) as well as an updated `ASHBORNE_DATABASE_URL`.

## Production checklist

- Set `ASHBORNE_ENVIRONMENT=production`, `ASHBORNE_AUTO_CREATE_TABLES=false`, and `ASHBORNE_SEED_DEVELOPMENT=false`; omit all three seed-password variables and the demo bootstrap secret, and remove `ASHBORNE_ALLOW_INSECURE_HTTP` from every agent environment.
...
- Use a managed or separately backed-up PostgreSQL service and a private authenticated Redis service. Give ASHBORNE a non-superuser database role limited to its schema.
- Inject secrets from a secret manager. Rotate the JWT signing key with an explicit session invalidation plan.
- Restrict `ASHBORNE_CORS_ORIGINS` and `ASHBORNE_TRUSTED_HOSTS` to exact deployed origins and hostnames.
...
Read `CHANGELOG.md`, run all tests, take a backup, migrate a staging copy, and exercise login/enrollment/heartbeat/task/audit flows. Roll the API and UI together when their contracts change. Agents should be backward compatible within a documented version window; ASHBORNE never silently installs agent updates.
